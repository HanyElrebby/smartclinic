...
import com.smartinn.smartclinic.service.VisitService;
...
/**
 * REST controller for managing {@link com.smartinn.smartclinic.domain.Visit}.
 */
@RestController
@RequestMapping("/api")
public class VisitResource {
    ...
    @GetMapping("/visits/waited")
    public ResponseEntity<List<Visit>> getAllVisitsWaited() {
        log.debug("REST request to get a page of Visits");
        List<Visit> page = visitService.findAllWaited();
    ...
    public ResponseEntity<List<Visit>> getAllVisitsByPatientId(@PathVariable String date, Pageable pageable) {
        log.debug("REST request to get a page of Visits");
        Page<Visit> page = visitService.findAllByDate(date, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
